...
The merged application uses `app.services.llm.request_explanation` through
`/api/scenarios/{id}/explanation` (or its `/evaluate` alias). Numerical results
are saved first by `/submit`, independently of AI. Both endpoints use the same
session ownership, background task and retry protections; see
[integration.md](integration.md) for the request and polling contract.

The active adapter requires both `OPENAI_API_KEY` and `OPENAI_MODEL`, uses
2,200 output tokens and one retry for transient failures. It shares the base URL
and timeout validation below; its per-attempt timeout is capped at 40 seconds
for the 90-second task lease (Compose defaults to 20). Redirects are disabled.

The standalone adapter described below is retained for direct Python callers;
it is not a second scenario persistence path. Its output limits apply only to
`explain_evaluation`, not to the active application adapter.

...
test('real API: five decisions, reference score, reload, AI failure, copy', async ({ page }) => {
  ...
  await expect.poll(async () => Number(await page.getByRole('meter', { name: 'Качество жизни Астаны' }).getAttribute('aria-valuenow'))).toBeCloseTo(52.56, 2)
  ...
  await expect.poll(async () => Number(await page.getByRole('meter', { name: 'Качество жизни Астаны' }).getAttribute('aria-valuenow'))).toBeCloseTo(56.54, 2)
  await page.reload()
  await expect(page.getByTestId('score')).toHaveText('56,54')
...
test('session isolation and mobile layout', async ({ page, browser }) => {
  ...
  const signals = page.getByRole('region', { name: 'Предложения жителей' })
  await expect(signals.locator('.public-proposal-card')).toHaveCount(2)
  await expect(signals.getByText('Подавлено', { exact: true }).first()).toBeVisible()
  await expect(page.locator('.city-live-player')).toHaveCount(0)
  const directions = page.getByRole('navigation', { name: 'Пять направлений управления' })
  await expect(directions.getByRole('link')).toHaveCount(5)
  await directions.getByRole('link', { name: 'Экология', exact: true }).click()
  await expect(page).toHaveURL(/#direction-ecology$/)
  await expect(page.locator('#direction-ecology')).toBeInViewport()
  expect(await page.evaluate(() => document.documentElement.scrollWidth)).toBeLessThanOrEqual(390)
  const other = await browser.newContext()
  const secondPage = await other.newPage()
  await secondPage.goto(`${new URL(page.url()).origin}/legacy-simulator`)
